...
def batch_laplacian(v, f):
    ...
    num_b = tf.shape(input=v)[0]
    num_v = tf.shape(input=v)[1]
    num_f = tf.shape(input=f)[0]
    ...
    cot_a = -1 * tf.reduce_sum(input_tensor=ab * ca, axis=2) / tf.sqrt(tf.reduce_sum(input_tensor=tf.linalg.cross(ab, ca) ** 2, axis=-1))
    cot_b = -1 * tf.reduce_sum(input_tensor=bc * ab, axis=2) / tf.sqrt(tf.reduce_sum(input_tensor=tf.linalg.cross(bc, ab) ** 2, axis=-1))
    cot_c = -1 * tf.reduce_sum(input_tensor=ca * bc, axis=2) / tf.sqrt(tf.reduce_sum(input_tensor=tf.linalg.cross(ca, bc) ** 2, axis=-1))
    ...
    L = sp_L_raw[0]
    for i in range(1, 6):
        L = tf.sparse.add(a=L, b=sp_L_raw[i])
    ...
    return tf.sparse.add(a=L, b=dia)
